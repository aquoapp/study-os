...
const UNITS = [
  {
    ...
    body: [
      'Releer subrayados produce una sensación de dominio que no se corresponde con lo que luego se recuerda. El texto resulta familiar, y esa familiaridad se confunde con saberlo.',
      'La alternativa es sencilla: cerrar el material e intentar recuperar lo leído sin mirarlo. El esfuerzo de recuperar es justamente lo que consolida el recuerdo, y por eso una respuesta incompleta enseña más que otra lectura pasiva.',
      'En la práctica basta con una regla: después de cada bloque, escribe de memoria las tres ideas principales antes de comprobar si acertaste.',
  ...
  },
  {
    ...
    body: [
      'El mismo número de horas rinde de forma muy distinta según cómo se reparta. Concentrar todo el estudio de un tema en una sesión larga permite responder bien esa tarde y muy mal dos semanas después.',
      'Separar las sesiones obliga a recuperar lo aprendido cuando ya se ha empezado a olvidar, y ese es el momento en que la recuperación deja más huella. El olvido parcial no es un fallo del método: es la condición que lo hace funcionar.',
      'Regla práctica: vuelve a un tema cuando empieces a dudar de él, no cuando aún lo tengas fresco.',
    ].join(String.fromCharCode(10, 10)),
  },
];
