...
| **D-17** · Las filas de auditoría de `ingest.staged_items` e `ingest.promotions` sobreviven a la purga de los packs sintéticos en STAGING (por diseño: la purga elimina contenido, no auditoría). Cada roundtrip las elimina con el esquema y cada ronda de pruebas vuelve a crearlas; el recuento exacto al cierre consta en la evidencia (catálogo de STAGING), sin contenido canónico enlazado. Casi todas son GENERATED; las únicas de clase OFFICIAL o VERIFIED proceden de dos pruebas negativas con filas visiblemente sintéticas (una fuente titulada «fixture: fuente que se declara oficial», sin versión ni contenido, para exigir el checksum; un borrador «fixture: borrador verificado» para probar que la purga se niega), retiradas por el servidor en la misma prueba | Nueva · aceptable · política de retención de auditoría pendiente (entrada de Phase 1B / operación) |
...
| Filas de pruebas negativas | Una fuente sintética que **se declara** OFFICIAL (título «fixture: fuente que se declara oficial», `source_type: FIXTURE`, sin versión con contenido) para probar que una versión OFFICIAL exige checksum; un borrador de representación VERIFIED («fixture: borrador verificado», `DRAFT`) para probar que la purga se niega mientras el pack tenga una fila no GENERATED | Publicadas por RPC de servicio y **retiradas por el servidor en la misma prueba**; solo dejan su promoción en la auditoría (D-17) |
