...
> **Nota de supersesión parcial · 2026-09-07.** Este ADR sigue `PROPOSED` en conjunto. Dos puntos cambian de propietario:
>
> - **Punto 4** (claves de respuesta fuera del Data API) queda **subordinado a ADR-006**, propietario normativo único de SD-007 (`ACCEPTED · NOT IMPLEMENTED`). Este ADR **referencia** esa frontera; no la duplica ni la redefine.
> - **Punto 5** (`exam_sittings` / `exam_occurrences`, BD-05) queda **superseded por ADR-010**, propietario normativo único de BD-05 / SD-001 (`ACCEPTED · NOT IMPLEMENTED`).
>
...
4. **[SUBORDINADO a ADR-006 · definición normativa allí]** **Las claves de respuesta no se exponen al cliente.** `answer_key_versions` queda fuera del esquema accesible por el Data API; la corrección ocurre en servidor (INV-101). *Ver ADR-006.*
5. **[SUPERSEDED por ADR-010 · texto histórico]** **Se añaden entidades de examen** —`exam_sittings` (convocatoria/sesión) y `exam_occurrences` (pregunta canónica × modelo × sección × `display_no` × fichero fuente)— para representar A/B como permutaciones de una misma pregunta semántica (CORP-001) y para soportar preguntas de reserva. Verificado sobre el corpus: la letra correcta no diverge entre modelos en ninguna de las 270 preguntas, por lo que la clave se mantiene a nivel de pregunta canónica con `model` como metadato de procedencia. **Sujeto a BD-05.**
