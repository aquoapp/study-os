...
STATUS: PROPOSED · v1.2 (añadido el contrato de orden total; requería SD-015 — **SD-015 queda SUPERSEDED BY SD-018 / ADR-008 desde 2026-09-07**)
DATE: 2026-08-22
...
> **Nota de supersesión parcial · 2026-09-07.** Este ADR **no se acepta tal como está** y sigue `PROPOSED` en conjunto. Tres puntos quedan sustituidos por ADR aceptados, y su texto se conserva solo como historia, marcado como **no operativo**:
>
> - **Punto 6** (referencias polimórficas) → **superseded por ADR-007**, propietario normativo único de SD-006 (`ACCEPTED · NOT IMPLEMENTED`).
> - **Punto 4**, en su frase sobre `ON CONFLICT DO NOTHING` «en ambos niveles», y **punto 10** entero (`server_sequence` global, watermark global «sin huecos», SD-015) → **superseded por ADR-008**, propietario normativo único de SD-018 (`ACCEPTED · NOT IMPLEMENTED`). **SD-015 queda `SUPERSEDED BY SD-018 / ADR-008`.** El sustituto normativo del contrato de orden e idempotencia es ADR-008.
>
> El resto —taxonomía, `event_id` como clave de idempotencia, inmutabilidad, `submitted_event_id` como deduplicación principal con `attempt_number` derivado, selección frente a envío, eventos adicionales, `diagnostic_run_id`, respuesta en blanco— sigue `PROPOSED` y sin aprobar. Registro de decisión: `STUDY_OS_Phase_0_Human_Decision_Packet_v1.0.md` (SHA-256 `6772d7021a2c1e3513d1bb7900cb1e1f1131e7f71e9386cd1e6533c695ecad7d`).
...
4. **Normalización e idempotencia del intento (corregido en v1.1):** `question_attempts.submitted_event_id` es **NOT NULL, UNIQUE y FK a `learning_events.event_id`**, y es el **mecanismo principal de deduplicación**. `attempt_number` queda como contador derivado asignado por el servidor, nunca como clave de idempotencia: el cliente no puede conocerlo de forma fiable en un reintento y dos dispositivos pueden calcular el mismo valor para eventos distintos.
   **[NO OPERATIVO · superseded por ADR-008 · texto histórico]** La normalización ocurre en la misma transacción que acepta el evento, con `ON CONFLICT DO NOTHING` en ambos niveles. `attempt_number` se calcula bajo bloqueo por par (user, question) o por secuencia, evitando la carrera de `SELECT MAX()+1`. *(ADR-008 fija el orden real: bloqueo del contador, comprobación de `submitted_event_id`, triple coincidencia, y ningún `ON CONFLICT DO NOTHING` después de asignar.)*
...
6. **[SUPERSEDED por ADR-007 · texto histórico]** **Referencias polimórficas:** `session_items` y `planner_items` usan columnas nullable por tipo con FK real y un CHECK de exclusividad (`exactly one non-null`), en lugar de un `item_ref_id` sin integridad. La base de datos debe poder garantizar el invariante de CDEM §23.
