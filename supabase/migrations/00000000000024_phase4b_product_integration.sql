-- ---------------------------------------------------------------------------
-- STUDY OS · Migración 24 · Phase 4B · Product Integration
--
-- `docs/PHASE_4B_PREAUTHORIZATION.md` §11 (lista cerrada) · ADR-013 (autoridad de duración) ·
-- ADR-012 anexo v1.5 · `docs/PLANNER_CONTRACT.md` v1.4 superseded en §C, §I.2, §I.3, §J, §O, §T,
-- §V y §Z · INV-106, INV-113, INV-116, INV-117, INV-118 · EC-009, EC-019 · SD-023.
--
-- Decisiones humanas que materializa:
--
--   - **P4-D2** · autoridad de duración híbrida. La unidad trae metadato de autoría versionado y
--     fijado a la versión exacta; la pregunta, una estimación gobernada en `planner_config`. La
--     ausencia **no se rellena**: excluye con `NO_DURATION_METADATA`.
--   - **P4B-D1** · `NOTHING_FITS` no crea ni ofrece acción fuera de presupuesto. No hay cambio de
--     esquema: la decisión vive en el motor puro y en la interfaz.
--   - **P4B-D2** · consumo de ejecución. Una ejecución cuya sesión alcanzó estado terminal **no se
--     reutiliza**: la petición siguiente escribe una sucesora. `start_planned_session` devuelve la
--     sesión existente **solo si está abierta**, y añade `RUN_ALREADY_CONSUMED`.
--   - **P4B-D3** · override del mismo día: tabla canónica más evento duradero, escritura solo de
--     servidor, día derivado en servidor desde la zona declarada.
--   - **Q-3 · INV-117** · `start_planned_session` copia `presented_learning_unit_version_id` y
--     `presented_representation_id`. Ese es todo el cambio: las comprobaciones
--     `REPRESENTATION_MISMATCH` de la frontera de eventos ya existen y se convierten en la guarda.
--   - **INV-118** · un tipo de evento cuya autoridad de producción es el servidor no es emitible
--     por un cliente. `public.append_learning_event` lo rechaza antes de cualquier escritura.
--
-- Lo que **no** materializa, y no por olvido: ningún esquema privado nuevo ni ampliado —`engine`,
-- `content` e `ingest` siguen sin exponer, ADR-011 intacto—; ninguna RPC invocable por cliente
-- nueva (la superficie sigue en dos); ninguna columna `run_type` (CDEM §17, SUPERSEDED);
-- ningún `priority_score`, peso, ratio, cuota, umbral, intervalo de repaso ni factor de
-- decaimiento; ninguna proyección nueva; ningún dato de contenido.
--
-- Rollback: supabase/migrations/down/00000000000024_phase4b_product_integration.down.sql
-- ---------------------------------------------------------------------------

-- 1 · P4-D2 · duración de la unidad · metadato de autoría fijado a la versión ------------------

-- Anulable **por decisión, no por descuido**: `NOT NULL` obligaría a rellenar las filas ya
-- publicadas, y todo valor rellenado sería un minuto inventado, que §2.5 prohíbe.
...
begin
  if not exists (
    ...
    where conname = 'learning_unit_versions_estimated_minutes_range'
      and conrelid = 'public.learning_unit_versions'::regclass
  ) then
    alter table public.learning_unit_versions
      add constraint learning_unit_versions_estimated_minutes_range
      check (estimated_minutes is null or estimated_minutes between 1 and 600);
  end if;
end;
$$;

comment on column public.learning_unit_versions.estimated_minutes is
  'ADR-013 · P4-D2 · estimación operativa de planificación, declarada por autoría y fijada a esta '
  'versión exacta. NO es un hecho de ciencia del aprendizaje, ni estimación de dominio, ni '
  'predicción personal, ni valor aprendido del historial. Anulable: la ausencia es una condición '
  'estructural veraz que excluye el candidato, nunca un valor por defecto.';

-- La duración de una versión publicada es inmutable como el resto de su contenido. Omitirla de
-- esta enumeración la dejaría mutable en silencio, que es el debilitamiento que EC-019 impide.
...
begin
  ...
  if old.status = 'PUBLISHED' then
    if new.learning_unit_id is distinct from old.learning_unit_id
       ...
       -- ADR-013 · corregir una duración equivocada acuña una versión nueva (OBS-4B-02).
    ...
    if new.status not in ('PUBLISHED', 'RETIRED') then
      raise exception 'H-FPS-1 · una versión publicada solo pasa a RETIRED'
        using errcode = 'restrict_violation';
    end if;
  end if;
  return new;
...
-- 2 · La ausencia se previene hacia delante en la frontera de ingestión ------------------------

-- `validate_staged_item` exige la clave para `learning_unit_version`; `publish_staged_item` la
-- traslada al insert. Ninguna de las dos rellena nada: exigir no es inventar.
create or replace function ingest.require_estimated_minutes(p jsonb)
returns void
language plpgsql
immutable
security invoker
...
as $$
declare
  raw text := p ->> 'estimated_minutes';
  value integer;
begin
  if not (p ? 'estimated_minutes') or raw is null then
    raise exception 'ADR-013 · una versión de unidad exige estimated_minutes: la duración no se rellena en silencio'
      using errcode = 'invalid_parameter_value';
  end if;
  begin
    value := raw::integer;
  exception when others then
    raise exception 'ADR-013 · estimated_minutes debe ser un entero: %', raw
      using errcode = 'invalid_parameter_value';
  end;
  if value < 1 or value > 600 then
    raise exception 'ADR-013 · estimated_minutes fuera de rango: %', value
      using errcode = 'invalid_parameter_value';
  end if;
end;
$$;
revoke all on function ingest.require_estimated_minutes(jsonb) from public, anon, authenticated, service_role;
grant execute on function ingest.require_estimated_minutes(jsonb) to service_role;

-- 3 · P4-D2 · duración de la pregunta · estimación gobernada en planner_config -----------------

-- §T · la lista blanca se **estrecha, no se abre**: admite exactamente una clave más, entera y
-- acotada. Pesos, ratios, porcentajes, cuotas, longitudes de ciclo, umbrales sobre recuentos de
-- evidencia, intervalos de repaso, semividas y factores de decaimiento siguen rechazados por
-- construcción.
alter table public.planner_config
  drop constraint if exists planner_config_document_whitelist;
alter table public.planner_config
...
alter table public.planner_config
  drop constraint if exists planner_config_document_complete;
alter table public.planner_config
  add constraint planner_config_document_complete check (
    ...
    -- La clave es opcional en el esquema para que `v1` siga siendo válida como historia, y
    -- obligatoria de hecho para planificar: sin ella el Planner no tiene duración de pregunta.
    and (
      not (document ? 'check_step_minutes')
      or (
        jsonb_typeof(document -> 'check_step_minutes') = 'number'
        and (document ->> 'check_step_minutes') ~ '^[0-9]+$'
        and (document ->> 'check_step_minutes')::integer between 1 and 60
      )
    )
  );

-- v2 · promovida con autor, motivo y evidencia. `v1` pasa a SUPERSEDED y se conserva: las
-- ejecuciones de Phase 4A la nombran.
update public.planner_config
...
select 'v2', 'ACTIVE',
  jsonb_build_object(
    ...
    'exclusion_reasons', jsonb_build_array(
      ...
      'COMPLETED_TODAY', 'POSITIVE_NO_REVIEW_POLICY', 'OVER_BUDGET', 'NO_DURATION_METADATA'
    ),
    'plan_day_policy', 'LEARNER_DECLARED_TIMEZONE',
    'check_step_minutes', 3
  ),
  'phase-4b-build-authorization',
  'P4-D2 · estimacion gobernada del paso COMPROBAR y alta de NO_DURATION_METADATA',
  now(),
  'phase-4b-build-authorization',
  jsonb_build_object(
    'contract', 'docs/PHASE_4B_PREAUTHORIZATION.md §2.2',
    'adr', 'ADR-013',
    'decisions', jsonb_build_array('P4-D2'),
    'note', 'estimacion operativa de planificacion, independiente de la persona, identica para '
            'toda pregunta en v1. No usa historial, response_ms, confianza ni dificultad.'
  )
where not exists (select 1 from public.planner_config where version = 'v2');

-- 4 · Procedencias ampliadas ------------------------------------------------------------------

alter table public.planner_runs
  drop constraint if exists planner_runs_duration_provenance;
alter table public.planner_runs
  add constraint planner_runs_duration_provenance
  check (duration_provenance in ('FIXTURE', 'HYBRID_V1'));

alter table public.planner_runs
  drop constraint if exists planner_runs_budget_source;
alter table public.planner_runs
  add constraint planner_runs_budget_source
  check (budget_source in ('TODAY_OVERRIDE', 'WEEKLY_ENTRY', 'DEFAULT_DAILY'));

-- 5 · P4B-D3 · override del tiempo de hoy -----------------------------------------------------

create table if not exists public.learner_day_overrides (
  user_id uuid not null references public.profiles (id) on delete cascade,
  plan_day date not null,
  minutes integer not null,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint learner_day_overrides_pk primary key (user_id, plan_day),
  -- **El cero es dato.** Un override de 0 es válido y produce `ZERO_TIME`. Ningún acoplamiento
  -- con `default_daily_minutes`, cuyo mínimo de 5 pertenece a otra declaración (INV-106).
  constraint learner_day_overrides_minutes_range check (minutes between 0 and 600)
);
comment on table public.learner_day_overrides is
  'P4B-D3 · declaración de la persona sobre el tiempo de UN día concreto. Estado canónico mutable '
  'que el Planner lee directamente; su declaración duradera se registra además como '
  'TODAY_OVERRIDE_SET. NO es una proyección autoritativa (INV-113): es una declaración de la '
  'persona, y su camino de escritura es solo de servidor por integridad, no por ser proyección.';

alter table public.learner_day_overrides enable row level security;
alter table public.learner_day_overrides force row level security;
revoke all on public.learner_day_overrides from public, anon, authenticated, service_role;

-- La persona **lee** su propia declaración; no la escribe directamente. La escritura pasa por la
-- función de servidor, que deriva el día y emite el evento en la misma transacción.
drop policy if exists learner_day_overrides_select_own on public.learner_day_overrides;
create policy learner_day_overrides_select_own
  on public.learner_day_overrides for select to authenticated
  using ((select auth.uid()) = user_id);
grant select on public.learner_day_overrides to authenticated;
grant select, insert, update on public.learner_day_overrides to service_role;

-- 6 · Presupuesto con override ----------------------------------------------------------------

-- §I.2 · precedencia: override del mismo día > entrada explícita del día de la semana
...
as $$
  select
    ...
    case
      when o.minutes is not null then 'TODAY_OVERRIDE'
      when s.weekly_availability_json ? k.key then 'WEEKLY_ENTRY'
      else 'DEFAULT_DAILY'
    end
  ...
  left join public.learner_day_overrides o
    on o.user_id = s.user_id and o.plan_day = p_plan_day
...
-- 7 · INV-118 · tipos de evento cuya autoridad de producción es el servidor --------------------

create or replace function ingest.event_type_is_server_only(p_type text)
returns boolean
language sql
immutable
security invoker
set search_path = ''
as $$
  -- Espeja `serverOnlyEventTypes` de packages/domain/src/authority-registry.json. Una prueba
  -- compara las dos listas para que no deriven.
  select p_type in ('TODAY_OVERRIDE_SET', 'AVAILABILITY_CHANGED');
$$;
revoke all on function ingest.event_type_is_server_only(text) from public, anon, authenticated, service_role;
grant execute on function ingest.event_type_is_server_only(text) to service_role;

-- `public.append_learning_event` es un envoltorio delgado concedido a `authenticated`. Sin este
-- rechazo, en cuanto `TODAY_OVERRIDE_SET` tiene contrato de campos un cliente podría crear una
-- declaración en la historia **sin fila canónica correspondiente**: historia y estado en
-- desacuerdo, que es justo lo que «tabla + evento» existe para evitar (INV-118).
...
as $$
begin
  if p_event is not null and jsonb_typeof(p_event) = 'object'
     and ingest.event_type_is_server_only(p_event ->> 'event_type') then
    raise exception 'STUDY_OS_EVENT · SERVER_ONLY_EVENT_TYPE · %', p_event ->> 'event_type'
...
comment on function public.append_learning_event(jsonb) is
  'H-P2-3 · RPC invocable por cliente (authority-registry.json · clientInvokableRpcs). Acepta un '
  'evento del usuario autenticado y devuelve posición, hash y, tras existir el intento, su '
  'resultado (INV-101: nunca antes del envío). INV-118 · rechaza todo tipo de evento cuya '
  'autoridad de producción es el servidor, antes de cualquier escritura.';
...
-- 8 · Contrato de campos de TODAY_OVERRIDE_SET ------------------------------------------------
...
as $$
  select case p_type
    ...
    when 'TODAY_OVERRIDE_SET' then '{"scope":"user","required":{"plan_day":"string","minutes":"integer"},"optional":{}}'::jsonb
    else null
  end;
$$;
...
-- 9 · Frontera de escritura del override ------------------------------------------------------

-- **Deriva el día de plan en servidor** desde `profiles.timezone`: el cliente nunca nombra la
-- fecha, por la misma autoridad que impide que `client_created_at` elija representación o clave
-- (SD-023). Solo rol de servicio, invocada desde una acción de servidor con identidad verificada
-- (INV-116). **La superficie de RPC invocable por cliente permanece en dos.**
create or replace function public.set_today_override(p_user uuid, p_minutes integer)
returns jsonb
language plpgsql
volatile
security definer
...
begin
  if p_user is null or p_minutes is null then
    raise exception 'STUDY_OS_OVERRIDE · PAYLOAD_MALFORMED' using errcode = 'invalid_parameter_value';
  end if;
  if p_minutes < 0 or p_minutes > 600 then
    raise exception 'STUDY_OS_OVERRIDE · MINUTES_OUT_OF_RANGE · %', p_minutes
      using errcode = 'invalid_parameter_value';
  end if;

  select p.timezone into tz from public.profiles p where p.id = p_user;
  if tz is null then
    raise exception 'STUDY_OS_OVERRIDE · TIMEZONE_REQUIRED' using errcode = 'object_not_in_prerequisite_state';
  end if;
  day := public.planner_plan_day(tz);

  insert into public.learner_day_overrides (user_id, plan_day, minutes)
  values (p_user, day, p_minutes)
  on conflict (user_id, plan_day) do update
    set minutes = excluded.minutes, updated_at = now();

  -- Estado canónico **más** evento de declaración duradero, en la misma transacción. El evento es
  -- autoritativo de servidor (INV-118): un cliente no puede crearlo con independencia del cambio.
  perform ingest.append_learning_event(p_user, jsonb_build_object(
    'event_id', event_id,
    'event_type', 'TODAY_OVERRIDE_SET',
    'schema_version', 1,
    'client_created_at', now(),
    'payload', jsonb_build_object('plan_day', day::text, 'minutes', p_minutes)
  ));

  return jsonb_build_object('planDay', day, 'minutes', p_minutes, 'source', 'TODAY_OVERRIDE');
end;
$$;
comment on function public.set_today_override(uuid, integer) is
  'P4B-D3 · única vía de escritura del override del tiempo de hoy: estado canónico más evento '
  'duradero en la misma transacción. Deriva el día de plan en servidor desde la zona declarada; '
  'el cliente nunca nombra la fecha (SD-023). Solo rol de servicio.';
revoke all on function public.set_today_override(uuid, integer) from public, anon, authenticated;
grant execute on function public.set_today_override(uuid, integer) to service_role;

-- 10 · Validación semántica del override en la frontera de eventos -----------------------------

-- El rango y la coherencia del día derivado se comprueban donde se comprueban las de
-- `AVAILABILITY_CHANGED`: dentro de la frontera, no en el llamador.
create or replace function ingest.validate_today_override_payload(p_user uuid, p_payload jsonb)
returns void
language plpgsql
stable
security invoker
...
begin
  begin
    minutes := (p_payload ->> 'minutes')::integer;
  exception when others then
    raise exception 'STUDY_OS_EVENT · TODAY_OVERRIDE_MALFORMED' using errcode = 'invalid_parameter_value';
  end;
  if minutes is null or minutes < 0 or minutes > 600 then
    raise exception 'STUDY_OS_EVENT · TODAY_OVERRIDE_MALFORMED' using errcode = 'invalid_parameter_value';
  end if;
  if (p_payload ->> 'plan_day') !~ '^\d{4}-\d{2}-\d{2}$' then
    raise exception 'STUDY_OS_EVENT · TODAY_OVERRIDE_MALFORMED' using errcode = 'invalid_parameter_value';
  end if;
  select p.timezone into tz from public.profiles p where p.id = p_user;
  if tz is null or public.planner_plan_day(tz) is distinct from (p_payload ->> 'plan_day')::date then
    raise exception 'STUDY_OS_EVENT · TODAY_OVERRIDE_DAY_MISMATCH' using errcode = 'invalid_parameter_value';
  end if;
end;
$$;
revoke all on function ingest.validate_today_override_payload(uuid, jsonb) from public, anon, authenticated, service_role;
grant execute on function ingest.validate_today_override_payload(uuid, jsonb) to service_role;

-- 11 · Frontera de eventos · alta de TODAY_OVERRIDE_SET ---------------------------------------

-- La función se reemite **completa y verbatim** desde la migración 18, con exactamente una rama
-- añadida al `case` de reglas de estado. PostgreSQL no admite parchear un cuerpo, y transcribirlo
-- a mano introduciría deriva: se copió del fichero congelado por script y se verificó su inicio y
-- su fin. Ningún otro comportamiento cambia.
...
begin
  ...
  case ev_type
    ...
    when 'TODAY_OVERRIDE_SET' then
      -- P4B-D3 §5.1 · rango y coherencia del día derivado, validados donde se validan los de
      -- AVAILABILITY_CHANGED: dentro de la frontera, no en el llamador.
      perform ingest.validate_today_override_payload(p_user, payload);
    else
...
comment on function ingest.append_learning_event(uuid, jsonb) is
  ...
  'efectos (ítem, intento, sesión, cursor, sync). Cualquier rechazo revierte todo sin consumir '
  'posición. Phase 4B · acepta TODAY_OVERRIDE_SET, cuya autoridad de producción es el servidor.';
revoke all on function ingest.append_learning_event(uuid, jsonb) from public, anon, authenticated, service_role;

-- 12 · Frontera de ingestión · la duración de la unidad entra por la única vía de publicación ---

-- Las dos funciones se reemiten **completas y verbatim** desde la migración 16, cada una con
-- exactamente una sustitución. Se copiaron del fichero congelado por script y se verificaron su
-- inicio y su fin. Ningún otro comportamiento cambia.
...
begin
  ...
  begin
    ...
    if item.kind = 'learning_unit_version' then
      -- ADR-013 · P4-D2 · la ausencia de duración se **previene hacia delante**. Exigirla aquí no
      -- rellena nada: las filas ya publicadas sin duración siguen sin ella y se excluyen con
      -- NO_DURATION_METADATA, que es la condición estructural veraz.
      perform ingest.require_estimated_minutes(p);
...
comment on function ingest.publish_staged_item(uuid) is
  ...
  '(PI-1A-6), inserta según el tipo y deja el ítem en PUBLISHED con su destino. Phase 4B · '
  'traslada estimated_minutes a la versión de unidad (ADR-013).';
...
-- 13 · Planner · consumo de ejecución, fidelidad de plan y duración desde la autoridad ---------

-- Las tres funciones se reemiten **completas y verbatim** desde la migración 23, cada una con las
-- sustituciones mínimas que las decisiones exigen. Se copiaron del fichero congelado por script.
...
begin
  ...
  return jsonb_build_object(
    ...
    -- ADR-013 §2.2 · estimación gobernada del paso COMPROBAR, de la versión ACTIVE. Nula si la
    -- configuración vigente no la declara: el Planner entonces no planifica y lo dice.
    'checkStepMinutes', (select (c.document ->> 'check_step_minutes')::integer
                           from public.planner_config c where c.status = 'ACTIVE'),
    'concepts', coalesce((
    ...
    'units', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        -- ADR-013 §2.1 · fijada a la versión exacta que el Planner selecciona. Nula es dato.
        'estimatedMinutes', uv.estimated_minutes
      ) order by u.id)
...
comment on function public.planner_context(uuid) is
  ...
  'de pack resuelta, día de plan en la zona declarada, presupuesto declarado con su procedencia '
  '(incluido el override del día), contenido elegible con su duración fijada a la versión, la '
  'estimación gobernada del paso COMPROBAR e ítems completados hoy. Solo rol de servicio.';
...
begin
  ...
  -- **P4B-D2** · una ejecución está **consumida** cuando existe una sesión suya en estado
  -- terminal. Una consumida **no se reutiliza**: la petición siguiente escribe una sucesora,
  -- aunque la entrada canónica no haya cambiado. La entrada canónica no codifica el consumo: el
  -- predicado se evalúa aquí, contra `study_sessions`, fuera de `PlannerInput`.
  if current_run.id is not null
     and current_run.plan_day = day
     and current_run.input_hash = p_payload ->> 'inputHash'
     and not exists (
       select 1 from public.study_sessions s
       where s.planner_run_id = current_run.id and s.status in ('COMPLETED', 'ABANDONED')
     ) then
...
comment on function public.create_planner_run(uuid, jsonb) is
  ...
  'su entrada en la misma transacción. P4B-D2 · una ejecución consumida no se reutiliza: la '
  'petición siguiente escribe una sucesora. Solo rol de servicio.';
...
begin
  ...
  -- §U.6 enmendada por **P4B-D2** · idempotente por ejecución **solo mientras la sesión está
  -- abierta**. Una sesión terminal significa que la ejecución está consumida: quien sostenga un
  -- identificador caducado recibe `RUN_ALREADY_CONSUMED` y vuelve a pedir plan, en lugar de
  -- recibir una sesión muerta.
  select s.id, s.status into existing from public.study_sessions s where s.planner_run_id = run.id;
  if existing.id is not null then
    if existing.status in ('COMPLETED', 'ABANDONED') then
      raise exception 'STUDY_OS_PLANNER · RUN_ALREADY_CONSUMED' using errcode = 'object_not_in_prerequisite_state';
    end if;
  ...
  for it in select * from public.planner_items i where i.run_id = run.id order by i.position loop
    -- **Q-3 · INV-117** · la versión de unidad y la representación que el Planner seleccionó son
    -- las que se presentan. Fijarlas en el insert basta: el trigger de fijación única las congela
    -- después, y las comprobaciones `REPRESENTATION_MISMATCH` de la frontera de eventos se
    -- convierten en la guarda sin escribir ninguna comprobación nueva. Una publicación posterior
    -- **nunca** sustituye la seleccionada.
    insert into public.session_items (
      session_id, user_id, item_type, learning_unit_id, question_id, sort_order, planned_minutes,
      presented_learning_unit_version_id, presented_representation_id
    ) values (
      session_id, p_user, it.item_type, it.learning_unit_id, it.question_id, it.position,
      it.planned_minutes,
      it.learning_unit_version_id, it.question_representation_id
    )
...
comment on function public.start_planned_session(uuid, uuid) is
  'Planner Contract §U.6 enmendada por P4B-D2 · arranque de una sesión planificada: comprueba '
  'propiedad, frescura, disponibilidad y ausencia de sesión abierta, y copia los ítems como '
  'instantánea fijando la versión y la representación seleccionadas (INV-117). Idempotente por '
  'ejecución mientras su sesión sigue abierta; una consumida produce RUN_ALREADY_CONSUMED.';
...
-- 14 · R-8 · el camino de escritura canónico de la disponibilidad emite su declaración ---------

-- R-8 cerrada a nivel de gobernanza y aquí implementada bajo el **mismo principio** que
-- TODAY_OVERRIDE_SET: cambio de estado canónico **más** evento de declaración duradero, con el
-- evento autoritativo de servidor, de modo que un cliente no pueda crearlo con independencia del
-- cambio de estado canónico.
--
-- Esto **no** clasifica la disponibilidad como evidencia de aprendizaje: es una declaración
-- duradera de la persona (T1 de la jerarquía de verdad).
--
-- Hasta hoy `AVAILABILITY_CHANGED` tenía contrato de campos y **no lo emitía nadie**: la acción de
-- onboarding escribía `learner_settings` directamente. Dejar esa vía abierta permitiría que el
-- estado canónico cambiase sin su declaración, que es justo el desacuerdo que R-8 cierra. Por eso
-- la escritura directa de cliente se retira y esta función pasa a ser la única vía.
create or replace function public.set_availability(
  p_user uuid,
  p_default_daily_minutes integer,
  p_weekly jsonb,
  p_diagnostic_preference text default null,
...
as $$
declare
  weekly jsonb := coalesce(p_weekly, '{}'::jsonb);
  event_id uuid := gen_random_uuid();
begin
  if p_user is null or p_default_daily_minutes is null then
    raise exception 'STUDY_OS_AVAILABILITY · PAYLOAD_MALFORMED' using errcode = 'invalid_parameter_value';
  end if;
  -- INV-106 · el mínimo de 5 pertenece a `default_daily_minutes`, no al override del día, cuyo
  -- cero es dato. Las dos declaraciones no se acoplan.
  if p_default_daily_minutes < 5 or p_default_daily_minutes > 600 then
    raise exception 'STUDY_OS_AVAILABILITY · MINUTES_OUT_OF_RANGE · %', p_default_daily_minutes
      using errcode = 'invalid_parameter_value';
  end if;
  if not public.weekly_availability_is_valid(weekly) then
    raise exception 'STUDY_OS_AVAILABILITY · WEEKLY_MALFORMED' using errcode = 'invalid_parameter_value';
  end if;
  if p_diagnostic_preference is not null and p_diagnostic_preference not in ('TAKE', 'SKIP') then
    raise exception 'STUDY_OS_AVAILABILITY · DIAGNOSTIC_PREFERENCE_INVALID' using errcode = 'invalid_parameter_value';
  end if;
  if not exists (select 1 from public.profiles p where p.id = p_user) then
    raise exception 'STUDY_OS_AVAILABILITY · PROFILE_NOT_FOUND' using errcode = 'insufficient_privilege';
  end if;

  insert into public.learner_settings (
    user_id, default_daily_minutes, weekly_availability_json, diagnostic_preference, reduced_motion
  ) values (
    p_user, p_default_daily_minutes, weekly, p_diagnostic_preference, coalesce(p_reduced_motion, false)
  )
  on conflict (user_id) do update set
    default_daily_minutes = excluded.default_daily_minutes,
    weekly_availability_json = excluded.weekly_availability_json,
    diagnostic_preference = coalesce(p_diagnostic_preference, public.learner_settings.diagnostic_preference),
    reduced_motion = coalesce(p_reduced_motion, public.learner_settings.reduced_motion),
    updated_at = now();
  ...
  perform ingest.append_learning_event(p_user, jsonb_build_object(
    ...
    'payload', jsonb_build_object(
      'default_daily_minutes', p_default_daily_minutes,
      'weekly_availability_json', weekly
    )
  ));

  return jsonb_build_object(
    'defaultDailyMinutes', p_default_daily_minutes,
    'weeklyAvailability', weekly
  );
end;
$$;
comment on function public.set_availability(uuid, integer, jsonb, text, boolean) is
  'R-8 · única vía de escritura de la disponibilidad declarada: estado canónico más evento '
  'AVAILABILITY_CHANGED en la misma transacción. No es evidencia de aprendizaje: es una '
  'declaración duradera de la persona. Solo rol de servicio.';
revoke all on function public.set_availability(uuid, integer, jsonb, text, boolean) from public, anon, authenticated;
grant execute on function public.set_availability(uuid, integer, jsonb, text, boolean) to service_role;

-- La persona sigue **leyendo** su fila. La escritura directa se retira: si pudiera escribirla sin
-- pasar por la función, el estado canónico cambiaría sin su declaración duradera (R-8).
revoke insert, update on public.learner_settings from authenticated;
...
grant select, insert, update on public.learner_settings to service_role;
