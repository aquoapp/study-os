-- ---------------------------------------------------------------------------
-- STUDY OS · Migración 13 · Frontera de ingestión: staging, cuarentena, publicación (N10)
--
-- ADR-011 · `ingest` no expuesto; las funciones de publicación son la ÚNICA vía de
--   escritura en tablas canónicas (SI-1A-6).
-- Master §31 · TA §8 · «Parsed content never jumps directly into published canonical tables»
-- INV-110 · OFFICIAL sin fuente primaria OFFICIAL queda en QUARANTINE (PI-1A-2)
-- EC-008 · PI-1A-4 · no existe ruta GENERATED → VERIFIED/OFFICIAL
-- PI-1A-6 · toda fila publicada enlaza la promoción que la creó
-- ADR-005 puntos 1, 6 (frontera) y 8 · disposición de Phase 1A
-- Rollback: supabase/migrations/down/00000000000013_ingest_boundary.down.sql
-- ---------------------------------------------------------------------------

create table if not exists ingest.staged_items (
  id uuid primary key default gen_random_uuid(),
  kind ingest.staged_item_kind not null,
  payload jsonb not null,
  status ingest.staged_item_status not null default 'RECEIVED',
  reason text,
  target_id uuid,
  promotion_id uuid references ingest.promotions (id),
  received_by text not null default current_user,
  received_at timestamptz not null default now(),
  validated_at timestamptz,
  published_at timestamptz,

  constraint staged_items_payload_object check (jsonb_typeof(payload) = 'object'),
  constraint staged_items_published_consistency
    check ((status = 'PUBLISHED') = (published_at is not null and target_id is not null and promotion_id is not null))
);

comment on table ingest.staged_items is
  'Master §31 · TA §8 · contenido recibido y validado antes de publicarse. Nunca se lee '
  'desde un cliente; nunca entra en una tabla canónica salvo por ingest.publish_staged_item.';

alter table ingest.promotions
  add column if not exists staged_item_id uuid references ingest.staged_items (id);

alter table ingest.staged_items enable row level security;
alter table ingest.staged_items force row level security;
revoke all on ingest.staged_items from public, anon, authenticated;
grant select, insert, update on ingest.staged_items to service_role;

-- Utilidades -------------------------------------------------------------------

create or replace function ingest.require_keys(p_payload jsonb, p_keys text[])
returns void
language plpgsql
immutable
security invoker
...
begin
  foreach k in array p_keys loop
    if not (p_payload ? k) or p_payload->k = 'null'::jsonb then
      raise exception 'payload sin la clave obligatoria "%"', k using errcode = 'invalid_parameter_value';
    end if;
  end loop;
end;
$$;

revoke all on function ingest.require_keys(jsonb, text[]) from public;

-- ADR-009 v1.1 §A · convención determinista de concept_key: <slug>-<hash8>.
create or replace function ingest.concept_key(p_pack_slug text, p_title text)
returns text
language plpgsql
immutable
security invoker
...
declare
  ...
  hash8 text;
begin
  -- NFKD y eliminación de marcas combinantes; minúsculas; solo [a-z0-9]; resto → '-'.
  folded := lower(regexp_replace(normalize(p_title, NFKD), '[\u0300-\u036f]', '', 'g'));
  slug := regexp_replace(folded, '[^a-z0-9]+', '-', 'g');
  slug := trim(both '-' from slug);
  if slug = '' then
    raise exception 'ADR-009 · el título no produce ningún carácter válido para el slug'
      using errcode = 'invalid_parameter_value';
  end if;
  if char_length(slug) > 40 then
    slug := left(slug, 40);
    -- Corte en frontera de palabra: se retira el último fragmento incompleto.
    if position('-' in slug) > 0 and right(slug, 1) <> '-' then
      slug := left(slug, char_length(slug) - char_length(split_part(reverse(slug), '-', 1)));
    end if;
    slug := trim(both '-' from slug);
  end if;
  hash8 := left(
    encode(extensions.digest(convert_to(p_pack_slug || E'\n' || normalize(p_title, NFC), 'UTF8'), 'sha256'), 'hex'),
    8
  );
  return slug || '-' || hash8;
end;
...
comment on function ingest.concept_key(text, text) is
  'ADR-009 v1.1 §A · <slug>-<hash8>: slug NFKD sin diacríticos (≤ 40, frontera de palabra) '
  'y 8 hex de SHA-256 sobre "<exam_pack.slug>\n<NFC(título)>". Exam-neutral y determinista.';

revoke all on function ingest.concept_key(text, text) from public;
grant execute on function ingest.concept_key(text, text) to service_role;

-- ¿La versión de fuente pertenece a una fuente OFFICIAL?
create or replace function ingest.source_version_is_official(p_source_version_id uuid)
returns boolean
language sql
stable
security invoker
...
as $$
  select exists (
    ...
    where sv.id = p_source_version_id and s.provenance_class = 'OFFICIAL'
  );
$$;

revoke all on function ingest.source_version_is_official(uuid) from public;
...
-- Validación (REJECTED · QUARANTINE · VALIDATED) --------------------------------
...
-- Publicación: la única vía a las tablas canónicas ----------------------------------
...
-- Purga de packs GENERATED sintéticos (higiene de fixtures) ---------------------
-- No es una ruta de promoción ni de borrado de contenido real: se niega si el pack
-- contiene una sola fila que no sea GENERATED. Activa la bandera transaccional que
-- los triggers de inmutabilidad consultan, y solo durante su transacción.
...
-- Envoltorios de servidor en `public` (ADR-011 punto 8) ------------------------------
-- Existen para que las herramientas de servidor y CI invoquen la frontera con rol de
-- servicio a través de PostgREST. Sin EXECUTE para anon, authenticated ni public: un
-- cliente ordinario recibe «permission denied». Registrados como RPC reservadas en
-- packages/domain/src/authority-registry.json.

create or replace function public.stage_item(p_kind text, p_payload jsonb)
returns uuid
language sql
security definer
set search_path = ''
as $$
  select ingest.stage_item(p_kind::ingest.staged_item_kind, p_payload);
$$;

create or replace function public.validate_staged_item(p_id uuid)
returns text
language sql
security definer
set search_path = ''
as $$
  select ingest.validate_staged_item(p_id)::text;
$$;

create or replace function public.publish_staged_item(p_id uuid)
returns uuid
language sql
security definer
set search_path = ''
as $$
  select ingest.publish_staged_item(p_id);
$$;

create or replace function public.copy_forward_question_concepts(p_from_version uuid, p_to_version uuid)
returns integer
language sql
security definer
set search_path = ''
as $$
  select ingest.copy_forward_question_concepts(p_from_version, p_to_version);
$$;

create or replace function public.purge_generated_pack(p_pack_id uuid)
returns jsonb
language sql
security definer
set search_path = ''
as $$
  select ingest.purge_generated_pack(p_pack_id);
$$;

revoke all on function public.stage_item(text, jsonb) from public, anon, authenticated;
revoke all on function public.validate_staged_item(uuid) from public, anon, authenticated;
revoke all on function public.publish_staged_item(uuid) from public, anon, authenticated;
revoke all on function public.copy_forward_question_concepts(uuid, uuid) from public, anon, authenticated;
revoke all on function public.purge_generated_pack(uuid) from public, anon, authenticated;
grant execute on function public.stage_item(text, jsonb) to service_role;
grant execute on function public.validate_staged_item(uuid) to service_role;
grant execute on function public.publish_staged_item(uuid) to service_role;
grant execute on function public.copy_forward_question_concepts(uuid, uuid) to service_role;
grant execute on function public.purge_generated_pack(uuid) to service_role;
